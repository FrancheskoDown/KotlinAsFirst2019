...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for ((name, phone) in mapB) {
        if (name !in result) result[name] = phone
        else
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    for ((name, price) in stockPrices) {
        if (name in result) result[name] = result[name]?.plus(price)!!.toDouble()
        else result[name] = price
        if (name in counter) counter[name] = counter[name]!!.plus(1)
        else counter[name] = 1
    }
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? {
    ...
    for ((name, product) in stuff) {
        if (product.first == kind && product.second <= minimalPrice) {
            stuffName = name
            minimalPrice = product.second
        }
    }
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    if ((word == " " && !chars.contains(' ')) || (chars.isEmpty() && word.isNotEmpty())) return false
    else if (chars.toString().toLowerCase().toSet().containsAll(word.toLowerCase().toSet())) return true
    return false
}
...
fun extractRepeats(list: List<String>): Map<String, Int> {
    ...
    for (name in list) {
        if (name in result) result[name] = result[name]!!.plus(1)
        else result[name] = 1
    }
...
fun hasAnagrams(words: List<String>): Boolean {
    if (words.size < 2 || words.isEmpty()) return false

    val data = words.toMutableSet()
    for (i in data) {
        for (k in data) {
            if (i == "" && k == "") return true
            if ((k.all { it in i } || i.all { it in k }) && k != i) return true
        }
    }
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    val numerals = mutableMapOf<Int, Set<Int>>()
    ...
    for (i in sortedList) {
        ...
        if (number == 0 && i == 0 && numerals[i]!!.size > 1) return Pair(numerals[i]!!.first(), numerals[i]!!.last())
...
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set<String> {
    ...
    for ((name, characteristics) in treasures) {
        names.add(name)
        masses.add(characteristics.first)
        costs.add(characteristics.second)
    }

    val backpack = Array<Pair<Int, MutableSet<String>>>((treasures.size + 1) * (capacity + 1)) { 0 to mutableSetOf() }

    for (thing in 1..names.size) {
        for (mass in 1..capacity) {
            val dontGetItem = backpack[(thing - 1) * (capacity + 1) + mass]
            if (masses[thing - 1] <= mass) {
    ...
    return backpack[treasures.size * (capacity + 1) + capacity].second.toSet()
}
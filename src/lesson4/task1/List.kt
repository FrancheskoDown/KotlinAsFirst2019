...
fun center(list: MutableList<Double>): MutableList<Double> {
    val mid = mean(list)
    for (counter in list.indices) list[counter] -= mid
...
fun polynom(p: List<Int>, x: Int): Int {
    ...
    for (counter in p.indices)
        result += p[counter] * x.toDouble().pow(counter).toInt()
    return result
}
...
fun accumulate(list: MutableList<Int>): MutableList<Int> {
    for (counter in 1 until list.size) list[counter] += list[counter - 1]
    return list
}
...
fun convertToString(n: Int, base: Int): String {
    val enter = convert(n, base)
    val exit = mutableListOf<String>()
    for (number in enter) {
        when (number) {
            in 0..9 -> exit.add(number.toString())
            in 10..35 -> exit.add(('a' + number - 10).toString())
        }
    }
